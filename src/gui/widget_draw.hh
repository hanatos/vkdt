...
, l, 99.293, 104.246, 98.617, 104.121, 98.004, 104.121, c, h,
...
104.699, 96.254, 104.57, 96.93, 104.57, 97.543, c, h,
...
, 85.234, 415.438, 87.801, c, 418, 90.363, 419.438, 93.84, 419.434, 97.465, c, h,
    ...
    const int    det)  // detail/how many points per bezier
{
  float vert[1000];
  int vert_cnt = 0;
  const int vert_max = sizeof(vert)/sizeof(vert[0])/2;
  float stack[10];
    ...
    if(!memcmp(cmd+i, &dt_draw_cmd_marker, sizeof(float)))
    {
      switch((int)cmd[++i])
      {
        ...
        case dt_draw_lineto:
          ...
          if(vert_cnt > vert_max - 1) return; // not enough verts
          vert[2*vert_cnt+0] = stack[sp-2];
          vert[2*vert_cnt+1] = stack[sp-1];
          sp -= 2; vert_cnt++;
          break;
        case dt_draw_curveto:
          if(sp < 6) return; // stack underflow
          if(vert_cnt > vert_max - 3) return; // not enough verts
          sp -= 6;
          for(int k=0;k<3;k++)
          {
            vert[2*vert_cnt+0] = stack[sp+2*k+0];
            vert[2*vert_cnt+1] = stack[sp+2*k+1];
            vert_cnt++;
          }
          if(vert_cnt < 4) return; // no moveto before?
          vert_cnt -= 4;
          float x[8];
          memcpy(x, vert+2*vert_cnt, sizeof(float)*8);
          for(int k=0;k<det;k++)
          { // interpolate cubic bezier
            const float t = k/(det-1.0f);
            const float t2 = t*t, t3 = t2*t;
            vert[2*vert_cnt+0] = (1.0f-3.0f*t+3.0f*t2-t3)*x[0] + (3.0f*t-6.0*t2+3.0*t3)*x[2] + 3.0f*(t2-t3)*x[4] + t3*x[6];
            vert[2*vert_cnt+1] = (1.0f-3.0f*t+3.0f*t2-t3)*x[1] + (3.0f*t-6.0*t2+3.0*t3)*x[3] + 3.0f*(t2-t3)*x[5] + t3*x[7];
            vert_cnt++;
          }
          break;
        case dt_draw_closepath:
          if(vert_cnt > 0)
          {
            ImGui::GetWindowDrawList()->AddPolyline((ImVec2*)vert, vert_cnt, IM_COL32_WHITE, true, vkdt.state.center_ht/500.0f);
            vert_cnt = 0;
          }
          break;
