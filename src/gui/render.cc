...
{ // anonymous gui state namespace
  ...
  void draw_arrow(float p[8], int feedback)
  {
    ...
    ImGui::GetWindowDrawList()->AddPolyline(
        (ImVec2 *)x, 30,
        feedback ? 0xffff8833 : IM_COL32_WHITE,
          ...
          if (copied_imgid != -1u)
          {
            ...
            if (ImGui::Button("paste history stack", size))
            {
              ...
              if (fin)
              {
                ...
                for (int i = 0; i < vkdt.db.selection_cnt; i++)
                {
                  if (sel[i] == copied_imgid)
                    continue; // don't copy to self
      ...
      if (connected && ImGui::Button("before this", fsize))
      {
        ...
        if (m_prev != -1)
        {
          ...
          if (c_our_in != -1 && c_new_out != -1 && c_new_in != -1)
          {
            ...
            err = 2ul << 32; // no input/output chain
        }
        else
          err = 2ul << 32; // no input/output chain
    ...
    else if (connected)
    {
        ...
        if (m_prev != -1 && cnt == 1)
        {
          ...
          if (c2 == 1)
          {
            ...
            if (c_our_out != -1 && c_our_in != -1)
            {
              ...
              err = 2ul << 32; // no input/output
          }
          else
            err = 3ul << 32; // no unique after
        }
        else if (m_prev == -1)
          err = 2ul << 32;
        else if (cnt != 1)
      ...
      if (ImGui::Button("disconnect", fsize))
      {
        ...
        if (m_prev != -1 && cnt > 0)
        {
          ...
          err = 2ul << 32; // no unique input/output chain
      }
    }
    else if (!connected)
    {
...
#include "pipe/graph-traverse.inc"
  ...
  void render_darkroom_pipeline()
  {
    ...
    for (int k = 0; k < graph->num_modules; k++)
      mod_id[k] = k;
    dt_module_t *const arr = graph->module;
    const int arr_cnt = graph->num_modules;
...
#include "pipe/graph-traverse.inc"
    ...
    for (int mi = graph->num_modules - 1; mi >= 0; mi--)
    {
      ...
      for (int k = graph->module[m].num_connectors - 1; k >= 0; k--)
      {
        if (dt_connector_input(graph->module[m].connector + k))
        {
          ...
          for (rev = 0; rev < graph->num_modules; rev++)
            if (mod_id[rev] == nid)
              break;
          if (rev == graph->num_modules + 1 || mod_id[rev] != nid)
            continue;
          // traverse mod_id list between mi and rev nid and get indentation level
          int ident = 0;
          if (mi < rev)
            for (int i = mi + 1; i < rev; i++)
            {
              mod_in[i]++;
              ident = MAX(mod_in[i], ident);
            }
          else
            for (int i = rev + 1; i < mi; i++)
            {
              mod_in[i]++;
              ident = MAX(mod_in[i], ident);
            }
  ...
  void render_darkroom()
  {
    { // center image view
      ...
      {
        // distinguish by type:
        switch (vkdt.graph_dev.module[vkdt.wstate.active_widget_modid].so->param[vkdt.wstate.active_widget_parid]->widget.type)
        {
        case dt_token("pers"):
        {
