...
dt_image_orientation_t dt_image_orientation_to_flip_bits(const int orient)
{
  switch(orient)
  {
...
#define FIND_EXIF_TAG(key) ((pos = exifData.findKey(Exiv2::ExifKey(key))) != exifData.end() && (pos)->size())
    ...
    if(ip->iso == 65535 || ip->iso == 0)
    {
      ...
      else if((!strcmp(ip->maker, "SONY") || !strcmp(ip->maker, "Canon"))
    ...
    if(FIND_EXIF_TAG("Exif.Photo.FocalLengthIn35mmFilm"))
    {
      ...
      if(focal_length_35mm > 0.0f && ip->focal_length > 0.0f)
        ip->crop = focal_length_35mm / ip->focal_length;
      else
        ip->crop = 1.0f;
    }
