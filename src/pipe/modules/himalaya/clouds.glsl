...
float volumetricShadow(in vec3 from, in float sundotrd, const int self_shadow_steps ) {
    float dd = CLOUDS_SHADOW_MARGE_STEP_SIZE;
    vec3 rd = SUN_DIR;
    float d = dd * .5;
    float shadow = 1.0;

    for(int s=0; s<self_shadow_steps; s++) {
        vec3 pos = from + rd * d;
...
vec4 renderClouds( vec3 ro, vec3 rd, inout float dist, const int march_steps, const int self_shadow_steps ) {
    if( rd.y < 0. ) {
    ...
    // raymarch
    float d = start;
    float dD = (end-start) / float(march_steps);
    ...
    for(int s=0; s<march_steps; s++) {
        ...
        if( alpha > 0. ) {
            ...
            vec3 S = (ambientLight + SUN_COLOR * (scattering * volumetricShadow(p, sundotrd, self_shadow_steps))) * alpha;
...
float volumetricShadowLayer(in vec3 from, in float sundotrd, const int self_shadow_steps ) {
...
vec4 renderCloudLayer( vec3 ro, vec3 rd, inout float dist, const int march_steps, const int self_shadow_steps ) {
    if( rd.y > 0. ) {
        return vec4(0,0,0,10);
    }
    ...
    for(int s=0; s<march_steps; s++) {
        ...
        if( alpha > 0. ) {
            ...
            vec3 S = .7 * (ambientLight +  SUN_COLOR * (scattering *
                  volumetricShadowLayer(p, sundotrd, self_shadow_steps))) * alpha;
            float dTrans = exp(-alpha * dD);
            vec3 Sint = (S - S * dTrans) * (1. / alpha);
            scatteredLight += transmittance * Sint; 
            transmittance *= dTrans;
