...
vec4 rgb_to_spectrum(vec3 srgb, vec4 lambda)
{
  ...
  vec4 rho_R = 0.974238*tanh((12.3964+1.97617*cos(6.86814*lambda))/((-141.502)+lambda)+asinh(asinh(pow(0.00154311*1.09295*lambda,106.109-1.83122*tan(lambda))-0.0246505)));
  // or:
  ...
  vec4 rho_G = tanh(lambda/(0.603055*cosh(lambda*(log((1.0/log(10.0))*log(1.22633*cos((-5.42359)*lambda)+1.11736*lambda))-0.0228972)-lambda)+264.94)+0.0151988);
