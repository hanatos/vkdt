#ifndef jddcnn_ixtrc_DENOX_MODULE_H
#define jddcnn_ixtrc_DENOX_MODULE_H
...
static int denox_read_source_ixtrc(dt_module_t* mod, void* mapped, dt_read_source_params_t* p) {
  if (p->node->kernel == dt_token("weights")) {
    FILE* f = dt_graph_open_resource(mod->graph, 0, "data/jddcnn-ixtrc.dat", "rb");
    if (!f) {
      snprintf(mod->graph->gui_msg_buf, sizeof(mod->graph->gui_msg_buf),
            "jddcnn: could not find \"data/jddcnn-ixtrc.dat\"");
      return 1;
    }
    fseek(f, 0, SEEK_END);
    ...
    if (size != expected_size) {
      snprintf(mod->graph->gui_msg_buf, sizeof(mod->graph->gui_msg_buf),
            "jddcnn: weight file \"data/jddcnn-ixtrc.dat\" has unexpected size!");
      fclose(f);
      return 1;
    }
    fseek(f, 0, SEEK_SET);
...
static void denox_create_nodes_ixtrc(dt_graph_t* graph, dt_module_t* module,
  ...
  const int64_t r85 = r83 * 64;
  const int64_t r88 = r49 * 128;
  const int64_t r90 = r18 + 15;
  const int64_t r91 = r90 / 16;
  const int64_t r94 = r18 + 3;
  const int64_t r95 = r94 / 4;
  const int64_t r96 = r30 + 7;
  const int64_t r97 = r96 / 8;
  const int64_t r98 = r15 + 31;
  const int64_t r99 = r98 / 32;
  const int64_t r100 = r83 * 32;
  const int64_t r104 = r15 + 23;
  ...
  dt_roi_t roi2 = {.wd = (uint32_t)(r100), .ht = 1};
  dt_roi_t roi3 = {.wd = (uint32_t)(r85), .ht = 1};
  dt_roi_t roi4 = {.wd = (uint32_t)(r88), .ht = 1};
  ...
  dt_roi_t roi21 = {.wd = (uint32_t)(r100), .ht = 1};
  dt_roi_t roi22 = {.wd = (uint32_t)(r100), .ht = 1};
  ...
  const int memory_pad_id = dt_node_add(graph, module, "jddcnn", "ixtrc6",
  ...
  const int direct_conv_cm_id = dt_node_add(graph, module, "jddcnn", "ixtrc16",
      1 * DT_LOCAL_SIZE_X, r113 * DT_LOCAL_SIZE_Y, r99,
  ...
  const int direct_conv_cm_1_id = dt_node_add(graph, module, "jddcnn", "ixtrc13",
      1 * DT_LOCAL_SIZE_X, r113 * DT_LOCAL_SIZE_Y, r99,
  ...
  const int direct_conv_cm_2_id = dt_node_add(graph, module, "jddcnn", "ixtrc12",
      1 * DT_LOCAL_SIZE_X, r97 * DT_LOCAL_SIZE_Y, r95,
  ...
  const int direct_conv_cm_3_id = dt_node_add(graph, module, "jddcnn", "ixtrc10",
      1 * DT_LOCAL_SIZE_X, r97 * DT_LOCAL_SIZE_Y, r91,
  ...
  const int direct_conv_cm_4_id = dt_node_add(graph, module, "jddcnn", "ixtrc11",
  ...
  const int direct_conv_cm_5_id = dt_node_add(graph, module, "jddcnn", "ixtrc17",
  ...
  const int direct_conv_cm_6_id = dt_node_add(graph, module, "jddcnn", "ixtrc20",
  ...
  const int direct_conv_cm_7_id = dt_node_add(graph, module, "jddcnn", "ixtrc14",
  ...
  const int direct_conv_cm_8_id = dt_node_add(graph, module, "jddcnn", "ixtrc7",
  ...
  const int direct_conv_cm_9_id = dt_node_add(graph, module, "jddcnn", "ixtrc21",
      1 * DT_LOCAL_SIZE_X, r70 * DT_LOCAL_SIZE_Y, r68,
  ...
  const int concat_conv_cm_id = dt_node_add(graph, module, "jddcnn", "ixtrc19",
      1 * DT_LOCAL_SIZE_X, r70 * DT_LOCAL_SIZE_Y, r68,
  ...
  const int direct_conv_cm_10_id = dt_node_add(graph, module, "jddcnn", "ixtrc20",
      1 * DT_LOCAL_SIZE_X, r70 * DT_LOCAL_SIZE_Y, r68,
  ...
  const int direct_conv_cm_11_id = dt_node_add(graph, module, "jddcnn", "ixtrc9",
      1 * DT_LOCAL_SIZE_X, r82 * DT_LOCAL_SIZE_Y, r80,
  ...
  const int concat_conv_cm_1_id = dt_node_add(graph, module, "jddcnn", "ixtrc18",
      1 * DT_LOCAL_SIZE_X, r82 * DT_LOCAL_SIZE_Y, r76,
  ...
  const int direct_conv_cm_12_id = dt_node_add(graph, module, "jddcnn", "ixtrc5",
      1 * DT_LOCAL_SIZE_X, r82 * DT_LOCAL_SIZE_Y, r76,
  ...
  const int direct_conv_cm_13_id = dt_node_add(graph, module, "jddcnn", "ixtrc4",
      1 * DT_LOCAL_SIZE_X, r97 * DT_LOCAL_SIZE_Y, r95,
  ...
  const int direct_conv_cm_14_id = dt_node_add(graph, module, "jddcnn", "ixtrc3",
      1 * DT_LOCAL_SIZE_X, r97 * DT_LOCAL_SIZE_Y, r95,
  ...
  const int direct_conv_cm_15_id = dt_node_add(graph, module, "jddcnn", "ixtrc8",
      1 * DT_LOCAL_SIZE_X, r97 * DT_LOCAL_SIZE_Y, r95,
  ...
  const int direct_conv_cm_16_id = dt_node_add(graph, module, "jddcnn", "ixtrc2",
  ...
  const int concat_conv_cm_2_id = dt_node_add(graph, module, "jddcnn", "ixtrc1",
  ...
  const int direct_conv_cm_17_id = dt_node_add(graph, module, "jddcnn", "ixtrc15",
  ...
  const int memory_slice_id = dt_node_add(graph, module, "jddcnn", "ixtrc0",
  ...
  graph->node[direct_conv_cm_14_id].connector[0].ssbo_offset = 4251776;
  graph->node[direct_conv_cm_14_id].connector[1].ssbo_offset = 4288640;
  graph->node[direct_conv_cm_15_id].connector[0].ssbo_offset = 4288704;
  graph->node[direct_conv_cm_15_id].connector[1].ssbo_offset = 4307136;
  graph->node[direct_conv_cm_16_id].connector[0].ssbo_offset = 4307200;
  graph->node[direct_conv_cm_16_id].connector[1].ssbo_offset = 4316416;
  graph->node[concat_conv_cm_2_id].connector[0].ssbo_offset = 4316448;
  graph->node[concat_conv_cm_2_id].connector[1].ssbo_offset = 4321056;
  graph->node[concat_conv_cm_2_id].connector[2].ssbo_offset = 4325664;
  graph->node[direct_conv_cm_17_id].connector[0].ssbo_offset = 4325696;
  graph->node[direct_conv_cm_17_id].connector[1].ssbo_offset = 4329152;
}
