    ...
    dt_write_sink_params_t *p)
{
  ...
  if(copy_exif)
  {
    ...
    char cmd[1024];
    if(sizeof(cmd) <= snprintf(cmd, sizeof(cmd), "\"%s/exiftool\" -TagsFromFile \"%s\" \"-all:all>all:all\" -Software=\"vkdt\" -ModifyDate=\"now\" -*image*= -*orientation*= -orientation=1 -overwrite_original \"%s\"",
