#ifndef oidn_gldrs_DENOX_MODULE_H
#define oidn_gldrs_DENOX_MODULE_H
#include "modules/api.h"
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>

static int denox_read_source_gldrs(dt_module_t* mod, void* mapped, dt_read_source_params_t* p) {
  if (p->node->kernel == dt_token("weights")) {
    FILE* f = dt_graph_open_resource(mod->graph, 0, "data/oidn-gldrs.dat", "rb");
    if (!f) {
      snprintf(mod->graph->gui_msg_buf, sizeof(mod->graph->gui_msg_buf),
            "oidn: could not find \"data/oidn-gldrs.dat\"");
      return 1;
    }
    fseek(f, 0, SEEK_END);
    const size_t size = ftell(f);
    const size_t expected_size = 641824;
    if (size != expected_size) {
      snprintf(mod->graph->gui_msg_buf, sizeof(mod->graph->gui_msg_buf),
            "oidn: weight file \"data/oidn-gldrs.dat\" has unexpected size!");
      fclose(f);
      return 1;
    }
    fseek(f, 0, SEEK_SET);
    fread(mapped, size, 1, f);
    fclose(f);
  }
  return 0;
}

static void denox_create_nodes_gldrs(dt_graph_t* graph, dt_module_t* module,
      uint64_t H, uint64_t W,
      int input_id, const char* input_connector,
      int output_id, const char* output_connector) {
  const int64_t r2 = W + 127;
  const int64_t r3 = r2 / 128;
  const int64_t r4 = ((W % 16) + 16) % 16;
  const int64_t r5 = r4 * -1;
  const int64_t r6 = r5 + 16;
  const int64_t r7 = ((r6 % 16) + 16) % 16;
  const int64_t r8 = W + r7;
  const int64_t r9 = r8 + 95;
  const int64_t r10 = r9 / 96;
  const int64_t r11 = r8 + 23;
  const int64_t r12 = r11 / 24;
  const int64_t r13 = r8 - 2;
  const int64_t r14 = r13 / 2;
  const int64_t r15 = r14 + 1;
  const int64_t r16 = r15 - 2;
  const int64_t r17 = r16 / 2;
  const int64_t r18 = r17 + 1;
  const int64_t r19 = r18 - 2;
  const int64_t r20 = r19 / 2;
  const int64_t r21 = r20 + 1;
  const int64_t r22 = r21 - 2;
  const int64_t r23 = r22 / 2;
  const int64_t r24 = r23 + 1;
  const int64_t r25 = r24 + 1151;
  const int64_t r26 = r25 / 1152;
  const int64_t r27 = r21 + 639;
  const int64_t r28 = r27 / 640;
  const int64_t r29 = r18 + 191;
  const int64_t r30 = r29 / 192;
  const int64_t r31 = r15 + 159;
  const int64_t r32 = r31 / 160;
  const int64_t r33 = r21 + 63;
  const int64_t r34 = r33 / 64;
  const int64_t r35 = r18 + 63;
  const int64_t r36 = r35 / 64;
  const int64_t r37 = r15 + 127;
  const int64_t r38 = r37 / 128;
  const int64_t r39 = r8 + 63;
  const int64_t r40 = r39 / 64;
  const int64_t r41 = ((H % 16) + 16) % 16;
  const int64_t r42 = r41 * -1;
  const int64_t r43 = r42 + 16;
  const int64_t r44 = ((r43 % 16) + 16) % 16;
  const int64_t r45 = H + r44;
  const int64_t r46 = r45 - 2;
  const int64_t r47 = r46 / 2;
  const int64_t r48 = r47 + 1;
  const int64_t r49 = r48 - 2;
  const int64_t r50 = r49 / 2;
  const int64_t r51 = r50 + 1;
  const int64_t r52 = r51 - 2;
  const int64_t r53 = r52 / 2;
  const int64_t r54 = r53 + 1;
  const int64_t r55 = r54 - 2;
  const int64_t r56 = r55 / 2;
  const int64_t r57 = r56 + 1;
  const int64_t r58 = r57 + 7;
  const int64_t r59 = r58 / 8;
  const int64_t r60 = r24 * r57;
  const int64_t r62 = r60 * 64;
  const int64_t r64 = r24 + 15;
  const int64_t r65 = r64 / 16;
  const int64_t r66 = r21 * r54;
  const int64_t r68 = r66 * 64;
  const int64_t r70 = r54 + 2;
  const int64_t r71 = r70 / 3;
  const int64_t r72 = r54 + 3;
  const int64_t r73 = r72 / 4;
  const int64_t r75 = r66 * 128;
  const int64_t r77 = r54 + 1;
  const int64_t r78 = r77 / 2;
  const int64_t r79 = r21 + 15;
  const int64_t r80 = r79 / 16;
  const int64_t r81 = r18 * r51;
  const int64_t r83 = r81 * 64;
  const int64_t r86 = r81 * 192;
  const int64_t r88 = r51 + 7;
  const int64_t r89 = r88 / 8;
  const int64_t r90 = r51 + 3;
  const int64_t r91 = r90 / 4;
  const int64_t r93 = r81 * 128;
  const int64_t r95 = r51 + 1;
  const int64_t r96 = r95 / 2;
  const int64_t r97 = r18 + 15;
  const int64_t r98 = r97 / 16;
  const int64_t r99 = r15 * r48;
  const int64_t r101 = r99 * 64;
  const int64_t r104 = r99 * 192;
  const int64_t r106 = r48 + 7;
  const int64_t r107 = r106 / 8;
  const int64_t r108 = r48 + 3;
  const int64_t r109 = r108 / 4;
  const int64_t r110 = r48 + 1;
  const int64_t r111 = r110 / 2;
  const int64_t r113 = r99 * 128;
  const int64_t r115 = r15 + 15;
  const int64_t r116 = r115 / 16;
  const int64_t r117 = r8 * r45;
  const int64_t r118 = r117 * 6;
  const int64_t r121 = r117 * 70;
  const int64_t r123 = r45 + 1;
  const int64_t r124 = r123 / 2;
  const int64_t r126 = r117 * 64;
  const int64_t r128 = r45 + 6;
  const int64_t r129 = r128 / 7;
  const int64_t r130 = r45 + 7;
  const int64_t r131 = r130 / 8;
  const int64_t r134 = r45 + 3;
  const int64_t r135 = r134 / 4;
  const int64_t r136 = r8 + 15;
  const int64_t r137 = r136 / 16;
  const int64_t r138 = H * W;
  const int64_t r139 = r138 * 6;
  dt_roi_t roi0 = {.wd = 641824, .ht = 1};
  dt_roi_t roi1 = {.wd = (uint32_t)(r139 / 2), .ht = 1};
  dt_roi_t roi2 = {.wd = (uint32_t)(r118), .ht = 1};
  dt_roi_t roi3 = {.wd = (uint32_t)(r126), .ht = 1};
  dt_roi_t roi4 = {.wd = (uint32_t)(r126), .ht = 1};
  dt_roi_t roi5 = {.wd = (uint32_t)(r104), .ht = 1};
  dt_roi_t roi6 = {.wd = (uint32_t)(r101), .ht = 1};
  dt_roi_t roi7 = {.wd = (uint32_t)(r86), .ht = 1};
  dt_roi_t roi8 = {.wd = (uint32_t)(r83), .ht = 1};
  dt_roi_t roi9 = {.wd = (uint32_t)(r75), .ht = 1};
  dt_roi_t roi10 = {.wd = (uint32_t)(r68), .ht = 1};
  dt_roi_t roi11 = {.wd = (uint32_t)(r62), .ht = 1};
  dt_roi_t roi12 = {.wd = (uint32_t)(r62), .ht = 1};
  dt_roi_t roi13 = {.wd = (uint32_t)(r62), .ht = 1};
  dt_roi_t roi14 = {.wd = 1, .ht = 1};
  dt_roi_t roi15 = {.wd = (uint32_t)(r75), .ht = 1};
  dt_roi_t roi16 = {.wd = (uint32_t)(r75), .ht = 1};
  dt_roi_t roi17 = {.wd = (uint32_t)(r93), .ht = 1};
  dt_roi_t roi18 = {.wd = (uint32_t)(r93), .ht = 1};
  dt_roi_t roi19 = {.wd = (uint32_t)(r113), .ht = 1};
  dt_roi_t roi20 = {.wd = (uint32_t)(r101), .ht = 1};
  dt_roi_t roi21 = {.wd = (uint32_t)(r126), .ht = 1};
  dt_roi_t roi22 = {.wd = (uint32_t)(r121), .ht = 1};
  dt_roi_t roi23 = {.wd = (uint32_t)(r126), .ht = 1};
  dt_roi_t roi24 = {.wd = (uint32_t)(r126), .ht = 1};
  dt_roi_t roi25 = {.wd = (uint32_t)(r118), .ht = 1};
  dt_roi_t roi26 = {.wd = (uint32_t)(r139 / 2), .ht = 1};
  int weights_id = dt_node_add(graph, module, "oidn", "weights",
      1, 1, 1, 0, NULL, 1, 
      "w", "source", "ssbo", "u8", &roi0);
  // memory_pad (memory_pad.comp)
  const uint32_t memory_pad_pc[6] = {(uint32_t)(r8), (uint32_t)(r45), 0, (uint32_t)(r7), 0, (uint32_t)(r44)};
  const int memory_pad_id = dt_node_add(graph, module, "oidn", "gldrs7",
      1 * DT_LOCAL_SIZE_X, r10 * DT_LOCAL_SIZE_Y, r129,
      24, (const int*)memory_pad_pc, 2, //
      "a", "read", "ssbo", "f16", &roi1, // HWC[3] : f16
      "b", "write", "ssbo", "u8", &roi2); // HWC[3] : f16
  // direct_conv (direct_conv.comp)
  const uint32_t direct_conv_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int direct_conv_id = dt_node_add(graph, module, "oidn", "gldrs9",
      1 * DT_LOCAL_SIZE_X, r137 * DT_LOCAL_SIZE_Y, r135,
      8, (const int*)direct_conv_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi2, // HWC[3] : f16
      "d", "write", "ssbo", "u8", &roi3); // CHWC8[32] : f16
  // direct_conv_1 (direct_conv.comp)
  const uint32_t direct_conv_1_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int direct_conv_1_id = dt_node_add(graph, module, "oidn", "gldrs11",
      1 * DT_LOCAL_SIZE_X, r137 * DT_LOCAL_SIZE_Y, r135,
      8, (const int*)direct_conv_1_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi3, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi4); // CHWC8[32] : f16
  // basic_pool (basic_pool.comp)
  const uint32_t basic_pool_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int basic_pool_id = dt_node_add(graph, module, "oidn", "gldrs13",
      4 * DT_LOCAL_SIZE_X, r32 * DT_LOCAL_SIZE_Y, r48,
      8, (const int*)basic_pool_pc, 2, //
      "a", "read", "ssbo", "u8", &roi4, // CHWC8[32] : f16
      "b", "write", "ssbo", "u8", &roi5); // CHWC8[32] : f16
  // direct_conv_2 (direct_conv.comp)
  const uint32_t direct_conv_2_pc[2] = {(uint32_t)(r15), (uint32_t)(r48)};
  const int direct_conv_2_id = dt_node_add(graph, module, "oidn", "gldrs14",
      1 * DT_LOCAL_SIZE_X, r116 * DT_LOCAL_SIZE_Y, r109,
      8, (const int*)direct_conv_2_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi5, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi6); // CHWC8[32] : f16
  // basic_pool_1 (basic_pool.comp)
  const uint32_t basic_pool_1_pc[2] = {(uint32_t)(r15), (uint32_t)(r48)};
  const int basic_pool_1_id = dt_node_add(graph, module, "oidn", "gldrs15",
      4 * DT_LOCAL_SIZE_X, r30 * DT_LOCAL_SIZE_Y, r96,
      8, (const int*)basic_pool_1_pc, 2, //
      "a", "read", "ssbo", "u8", &roi6, // CHWC8[32] : f16
      "b", "write", "ssbo", "u8", &roi7); // CHWC8[32] : f16
  // direct_conv_3 (direct_conv.comp)
  const uint32_t direct_conv_3_pc[2] = {(uint32_t)(r18), (uint32_t)(r51)};
  const int direct_conv_3_id = dt_node_add(graph, module, "oidn", "gldrs16",
      1 * DT_LOCAL_SIZE_X, r98 * DT_LOCAL_SIZE_Y, r91,
      8, (const int*)direct_conv_3_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi7, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi8); // CHWC8[32] : f16
  // basic_pool_2 (basic_pool.comp)
  const uint32_t basic_pool_2_pc[2] = {(uint32_t)(r18), (uint32_t)(r51)};
  const int basic_pool_2_id = dt_node_add(graph, module, "oidn", "gldrs20",
      4 * DT_LOCAL_SIZE_X, r28 * DT_LOCAL_SIZE_Y, r78,
      8, (const int*)basic_pool_2_pc, 2, //
      "a", "read", "ssbo", "u8", &roi8, // CHWC8[32] : f16
      "b", "write", "ssbo", "u8", &roi9); // CHWC8[32] : f16
  // direct_conv_4 (direct_conv.comp)
  const uint32_t direct_conv_4_pc[2] = {(uint32_t)(r21), (uint32_t)(r54)};
  const int direct_conv_4_id = dt_node_add(graph, module, "oidn", "gldrs21",
      1 * DT_LOCAL_SIZE_X, r80 * DT_LOCAL_SIZE_Y, r73,
      8, (const int*)direct_conv_4_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi9, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi10); // CHWC8[32] : f16
  // basic_pool_3 (basic_pool.comp)
  const uint32_t basic_pool_3_pc[2] = {(uint32_t)(r21), (uint32_t)(r54)};
  const int basic_pool_3_id = dt_node_add(graph, module, "oidn", "gldrs19",
      4 * DT_LOCAL_SIZE_X, r26 * DT_LOCAL_SIZE_Y, r57,
      8, (const int*)basic_pool_3_pc, 2, //
      "a", "read", "ssbo", "u8", &roi10, // CHWC8[32] : f16
      "b", "write", "ssbo", "u8", &roi11); // CHWC8[32] : f16
  // direct_conv_5 (direct_conv.comp)
  const uint32_t direct_conv_5_pc[2] = {(uint32_t)(r24), (uint32_t)(r57)};
  const int direct_conv_5_id = dt_node_add(graph, module, "oidn", "gldrs22",
      1 * DT_LOCAL_SIZE_X, r65 * DT_LOCAL_SIZE_Y, r59,
      8, (const int*)direct_conv_5_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi11, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi12); // CHWC8[32] : f16
  // direct_conv_6 (direct_conv.comp)
  const uint32_t direct_conv_6_pc[2] = {(uint32_t)(r24), (uint32_t)(r57)};
  const int direct_conv_6_id = dt_node_add(graph, module, "oidn", "gldrs22",
      1 * DT_LOCAL_SIZE_X, r65 * DT_LOCAL_SIZE_Y, r59,
      8, (const int*)direct_conv_6_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi12, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi13); // CHWC8[32] : f16
  // basic_upsample (basic_upsample.comp)
  const uint32_t basic_upsample_pc[2] = {(uint32_t)(r24), (uint32_t)(r57)};
  const int basic_upsample_id = dt_node_add(graph, module, "oidn", "gldrs17",
      4 * DT_LOCAL_SIZE_X, r34 * DT_LOCAL_SIZE_Y, r78,
      8, (const int*)basic_upsample_pc, 3, //
      "a", "read", "ssbo", "u8", &roi13, // CHWC8[32] : f16
      "b", "read", "ssbo", "u8", &roi9, // CHWC8[32] : f16
      "dummy", "write", "ssbo", "u8", &roi14);//
  // direct_conv_7 (direct_conv.comp)
  const uint32_t direct_conv_7_pc[2] = {(uint32_t)(r21), (uint32_t)(r54)};
  const int direct_conv_7_id = dt_node_add(graph, module, "oidn", "gldrs24",
      1 * DT_LOCAL_SIZE_X, r80 * DT_LOCAL_SIZE_Y, r71,
      8, (const int*)direct_conv_7_pc, 5, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi9, // CHWC8[64] : f16
      "d", "write", "ssbo", "u8", &roi15, // HWC[64] : f16
      "z0", "read", "ssbo", "u8", &roi14);//
  // direct_conv_8 (direct_conv.comp)
  const uint32_t direct_conv_8_pc[2] = {(uint32_t)(r21), (uint32_t)(r54)};
  const int direct_conv_8_id = dt_node_add(graph, module, "oidn", "gldrs6",
      1 * DT_LOCAL_SIZE_X, r80 * DT_LOCAL_SIZE_Y, r73,
      8, (const int*)direct_conv_8_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi15, // HWC[64] : f16
      "d", "write", "ssbo", "u8", &roi16); // CHWC8[64] : f16
  // basic_upsample_1 (basic_upsample.comp)
  const uint32_t basic_upsample_1_pc[2] = {(uint32_t)(r21), (uint32_t)(r54)};
  const int basic_upsample_1_id = dt_node_add(graph, module, "oidn", "gldrs5",
      8 * DT_LOCAL_SIZE_X, r36 * DT_LOCAL_SIZE_Y, r96,
      8, (const int*)basic_upsample_1_pc, 3, //
      "a", "read", "ssbo", "u8", &roi16, // CHWC8[64] : f16
      "b", "read", "ssbo", "u8", &roi7, // CHWC8[64] : f16
      "dummy", "write", "ssbo", "u8", &roi14);//
  // direct_conv_9 (direct_conv.comp)
  const uint32_t direct_conv_9_pc[2] = {(uint32_t)(r18), (uint32_t)(r51)};
  const int direct_conv_9_id = dt_node_add(graph, module, "oidn", "gldrs4",
      1 * DT_LOCAL_SIZE_X, r98 * DT_LOCAL_SIZE_Y, r89,
      8, (const int*)direct_conv_9_pc, 5, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi7, // CHWC8[96] : f16
      "d", "write", "ssbo", "u8", &roi17, // CHWC8[64] : f16
      "z0", "read", "ssbo", "u8", &roi14);//
  // direct_conv_10 (direct_conv.comp)
  const uint32_t direct_conv_10_pc[2] = {(uint32_t)(r18), (uint32_t)(r51)};
  const int direct_conv_10_id = dt_node_add(graph, module, "oidn", "gldrs12",
      1 * DT_LOCAL_SIZE_X, r98 * DT_LOCAL_SIZE_Y, r89,
      8, (const int*)direct_conv_10_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi17, // CHWC8[64] : f16
      "d", "write", "ssbo", "u8", &roi18); // CHWC8[64] : f16
  // basic_upsample_2 (basic_upsample.comp)
  const uint32_t basic_upsample_2_pc[2] = {(uint32_t)(r18), (uint32_t)(r51)};
  const int basic_upsample_2_id = dt_node_add(graph, module, "oidn", "gldrs3",
      8 * DT_LOCAL_SIZE_X, r38 * DT_LOCAL_SIZE_Y, r111,
      8, (const int*)basic_upsample_2_pc, 3, //
      "a", "read", "ssbo", "u8", &roi18, // CHWC8[64] : f16
      "b", "read", "ssbo", "u8", &roi5, // CHWC8[64] : f16
      "dummy", "write", "ssbo", "u8", &roi14);//
  // direct_conv_11 (direct_conv.comp)
  const uint32_t direct_conv_11_pc[2] = {(uint32_t)(r15), (uint32_t)(r48)};
  const int direct_conv_11_id = dt_node_add(graph, module, "oidn", "gldrs4",
      1 * DT_LOCAL_SIZE_X, r116 * DT_LOCAL_SIZE_Y, r107,
      8, (const int*)direct_conv_11_pc, 5, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi5, // CHWC8[96] : f16
      "d", "write", "ssbo", "u8", &roi19, // CHWC8[64] : f16
      "z0", "read", "ssbo", "u8", &roi14);//
  // direct_conv_12 (direct_conv.comp)
  const uint32_t direct_conv_12_pc[2] = {(uint32_t)(r15), (uint32_t)(r48)};
  const int direct_conv_12_id = dt_node_add(graph, module, "oidn", "gldrs23",
      1 * DT_LOCAL_SIZE_X, r116 * DT_LOCAL_SIZE_Y, r107,
      8, (const int*)direct_conv_12_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi19, // CHWC8[64] : f16
      "d", "write", "ssbo", "u8", &roi20); // HWC[32] : f16
  // basic_upsample_3 (basic_upsample.comp)
  const uint32_t basic_upsample_3_pc[2] = {(uint32_t)(r15), (uint32_t)(r48)};
  const int basic_upsample_3_id = dt_node_add(graph, module, "oidn", "gldrs10",
      1 * DT_LOCAL_SIZE_X, r12 * DT_LOCAL_SIZE_Y, r124,
      8, (const int*)basic_upsample_3_pc, 2, //
      "a", "read", "ssbo", "u8", &roi20, // HWC[32] : f16
      "b", "write", "ssbo", "u8", &roi21); // HWC[32] : f16
  // copy_transform (copy_transform.comp)
  const uint32_t copy_transform_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int copy_transform_id = dt_node_add(graph, module, "oidn", "gldrs2",
      1 * DT_LOCAL_SIZE_X, r40 * DT_LOCAL_SIZE_Y, r45,
      8, (const int*)copy_transform_pc, 2, //
      "a", "read", "ssbo", "u8", &roi21, // HWC[32] : f16
      "b", "write", "ssbo", "u8", &roi22); // HWC[35] : f16
  // copy_transform_1 (copy_transform.comp)
  const uint32_t copy_transform_1_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int copy_transform_1_id = dt_node_add(graph, module, "oidn", "gldrs1",
      1 * DT_LOCAL_SIZE_X, r40 * DT_LOCAL_SIZE_Y, r45,
      8, (const int*)copy_transform_1_pc, 3, //
      "a", "read", "ssbo", "u8", &roi2, // HWC[3] : f16
      "b", "read", "ssbo", "u8", &roi22, // HWC[35] : f16
      "dummy", "write", "ssbo", "u8", &roi14);//
  // direct_conv_13 (direct_conv.comp)
  const uint32_t direct_conv_13_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int direct_conv_13_id = dt_node_add(graph, module, "oidn", "gldrs0",
      1 * DT_LOCAL_SIZE_X, r137 * DT_LOCAL_SIZE_Y, r131,
      8, (const int*)direct_conv_13_pc, 5, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi22, // HWC[35] : f16
      "d", "write", "ssbo", "u8", &roi23, // CHWC8[32] : f16
      "z0", "read", "ssbo", "u8", &roi14);//
  // direct_conv_14 (direct_conv.comp)
  const uint32_t direct_conv_14_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int direct_conv_14_id = dt_node_add(graph, module, "oidn", "gldrs11",
      1 * DT_LOCAL_SIZE_X, r137 * DT_LOCAL_SIZE_Y, r135,
      8, (const int*)direct_conv_14_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi23, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi24); // CHWC8[32] : f16
  // direct_conv_15 (direct_conv.comp)
  const uint32_t direct_conv_15_pc[2] = {(uint32_t)(r8), (uint32_t)(r45)};
  const int direct_conv_15_id = dt_node_add(graph, module, "oidn", "gldrs18",
      1 * DT_LOCAL_SIZE_X, r137 * DT_LOCAL_SIZE_Y, r135,
      8, (const int*)direct_conv_15_pc, 4, //
      "a", "read", "ssbo", "u8", &roi0,// unknown
      "b", "read", "ssbo", "u8", &roi0,// unknown
      "c", "read", "ssbo", "u8", &roi24, // CHWC8[32] : f16
      "d", "write", "ssbo", "u8", &roi25); // HWC[3] : f16
  // memory_slice (memory_slice.comp)
  const uint32_t memory_slice_pc[6] = {0, 0, (uint32_t)(r8), (uint32_t)(r45), (uint32_t)(W), (uint32_t)(H)};
  const int memory_slice_id = dt_node_add(graph, module, "oidn", "gldrs8",
      1 * DT_LOCAL_SIZE_X, r3 * DT_LOCAL_SIZE_Y, H,
      24, (const int*)memory_slice_pc, 2, //
      "a", "read", "ssbo", "u8", &roi25, // HWC[3] : f16
      "b", "write", "ssbo", "f16", &roi26); // HWC[3] : f16
  if (input_connector == NULL) {
    dt_connector_copy(graph, module, input_id, memory_pad_id, 0);
  } else {
    dt_node_connect_named(graph, input_id, input_connector, memory_pad_id, "a");
  }
  dt_node_connect_named(graph, weights_id, "w", direct_conv_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_id, "b");
  dt_node_connect_named(graph, memory_pad_id, "b", direct_conv_id, "c");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_1_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_1_id, "b");
  dt_node_connect_named(graph, direct_conv_id, "d", direct_conv_1_id, "c");
  dt_node_connect_named(graph, direct_conv_1_id, "d", basic_pool_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_2_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_2_id, "b");
  dt_node_connect_named(graph, basic_pool_id, "b", direct_conv_2_id, "c");
  dt_node_connect_named(graph, direct_conv_2_id, "d", basic_pool_1_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_3_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_3_id, "b");
  dt_node_connect_named(graph, basic_pool_1_id, "b", direct_conv_3_id, "c");
  dt_node_connect_named(graph, direct_conv_3_id, "d", basic_pool_2_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_4_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_4_id, "b");
  dt_node_connect_named(graph, basic_pool_2_id, "b", direct_conv_4_id, "c");
  dt_node_connect_named(graph, direct_conv_4_id, "d", basic_pool_3_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_5_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_5_id, "b");
  dt_node_connect_named(graph, basic_pool_3_id, "b", direct_conv_5_id, "c");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_6_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_6_id, "b");
  dt_node_connect_named(graph, direct_conv_5_id, "d", direct_conv_6_id, "c");
  dt_node_connect_named(graph, direct_conv_6_id, "d", basic_upsample_id, "a");
  dt_node_connect_named(graph, basic_pool_2_id, "b", basic_upsample_id, "b");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_7_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_7_id, "b");
  dt_node_connect_named(graph, basic_pool_2_id, "b", direct_conv_7_id, "c");
  dt_node_connect_named(graph, basic_upsample_id, "dummy", direct_conv_7_id, "z0");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_8_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_8_id, "b");
  dt_node_connect_named(graph, direct_conv_7_id, "d", direct_conv_8_id, "c");
  dt_node_connect_named(graph, direct_conv_8_id, "d", basic_upsample_1_id, "a");
  dt_node_connect_named(graph, basic_pool_1_id, "b", basic_upsample_1_id, "b");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_9_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_9_id, "b");
  dt_node_connect_named(graph, basic_pool_1_id, "b", direct_conv_9_id, "c");
  dt_node_connect_named(graph, basic_upsample_1_id, "dummy", direct_conv_9_id, "z0");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_10_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_10_id, "b");
  dt_node_connect_named(graph, direct_conv_9_id, "d", direct_conv_10_id, "c");
  dt_node_connect_named(graph, direct_conv_10_id, "d", basic_upsample_2_id, "a");
  dt_node_connect_named(graph, basic_pool_id, "b", basic_upsample_2_id, "b");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_11_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_11_id, "b");
  dt_node_connect_named(graph, basic_pool_id, "b", direct_conv_11_id, "c");
  dt_node_connect_named(graph, basic_upsample_2_id, "dummy", direct_conv_11_id, "z0");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_12_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_12_id, "b");
  dt_node_connect_named(graph, direct_conv_11_id, "d", direct_conv_12_id, "c");
  dt_node_connect_named(graph, direct_conv_12_id, "d", basic_upsample_3_id, "a");
  dt_node_connect_named(graph, basic_upsample_3_id, "b", copy_transform_id, "a");
  dt_node_connect_named(graph, memory_pad_id, "b", copy_transform_1_id, "a");
  dt_node_connect_named(graph, copy_transform_id, "b", copy_transform_1_id, "b");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_13_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_13_id, "b");
  dt_node_connect_named(graph, copy_transform_id, "b", direct_conv_13_id, "c");
  dt_node_connect_named(graph, copy_transform_1_id, "dummy", direct_conv_13_id, "z0");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_14_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_14_id, "b");
  dt_node_connect_named(graph, direct_conv_13_id, "d", direct_conv_14_id, "c");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_15_id, "a");
  dt_node_connect_named(graph, weights_id, "w", direct_conv_15_id, "b");
  dt_node_connect_named(graph, direct_conv_14_id, "d", direct_conv_15_id, "c");
  dt_node_connect_named(graph, direct_conv_15_id, "d", memory_slice_id, "a");
  if (output_connector == NULL) {
    dt_connector_copy(graph, module, output_id, memory_slice_id, 1);
  } else {
    dt_node_connect_named(graph, memory_slice_id, "b", output_id, output_connector);
  }
  graph->node[direct_conv_id].connector[1].ssbo_offset = 4608;
  graph->node[direct_conv_1_id].connector[0].ssbo_offset = 4672;
  graph->node[direct_conv_1_id].connector[1].ssbo_offset = 23104;
  graph->node[basic_pool_id].connector[1].ssbo_offset = r113;
  graph->node[direct_conv_2_id].connector[0].ssbo_offset = 23168;
  graph->node[direct_conv_2_id].connector[1].ssbo_offset = 41600;
  graph->node[direct_conv_2_id].connector[2].ssbo_offset = r113;
  graph->node[basic_pool_1_id].connector[1].ssbo_offset = r93;
  graph->node[direct_conv_3_id].connector[0].ssbo_offset = 41664;
  graph->node[direct_conv_3_id].connector[1].ssbo_offset = 60096;
  graph->node[direct_conv_3_id].connector[2].ssbo_offset = r93;
  graph->node[basic_pool_2_id].connector[1].ssbo_offset = r68;
  graph->node[direct_conv_4_id].connector[0].ssbo_offset = 60160;
  graph->node[direct_conv_4_id].connector[1].ssbo_offset = 78592;
  graph->node[direct_conv_4_id].connector[2].ssbo_offset = r68;
  graph->node[direct_conv_5_id].connector[0].ssbo_offset = 78656;
  graph->node[direct_conv_5_id].connector[1].ssbo_offset = 97088;
  graph->node[direct_conv_6_id].connector[0].ssbo_offset = 97152;
  graph->node[direct_conv_6_id].connector[1].ssbo_offset = 115584;
  graph->node[direct_conv_7_id].connector[0].ssbo_offset = 115648;
  graph->node[direct_conv_7_id].connector[1].ssbo_offset = 189376;
  graph->node[direct_conv_8_id].connector[0].ssbo_offset = 189504;
  graph->node[direct_conv_8_id].connector[1].ssbo_offset = 263232;
  graph->node[direct_conv_9_id].connector[0].ssbo_offset = 263360;
  graph->node[direct_conv_9_id].connector[1].ssbo_offset = 373952;
  graph->node[direct_conv_10_id].connector[0].ssbo_offset = 374080;
  graph->node[direct_conv_10_id].connector[1].ssbo_offset = 447808;
  graph->node[direct_conv_11_id].connector[0].ssbo_offset = 447936;
  graph->node[direct_conv_11_id].connector[1].ssbo_offset = 558528;
  graph->node[direct_conv_12_id].connector[0].ssbo_offset = 558656;
  graph->node[direct_conv_12_id].connector[1].ssbo_offset = 595520;
  graph->node[direct_conv_13_id].connector[0].ssbo_offset = 595584;
  graph->node[direct_conv_13_id].connector[1].ssbo_offset = 618624;
  graph->node[direct_conv_14_id].connector[0].ssbo_offset = 618688;
  graph->node[direct_conv_14_id].connector[1].ssbo_offset = 637120;
  graph->node[direct_conv_15_id].connector[0].ssbo_offset = 637184;
  graph->node[direct_conv_15_id].connector[1].ssbo_offset = 641792;
}

#endif
